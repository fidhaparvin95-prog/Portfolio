...
if($firstname ==""){
    header("Location:adminhome.php?err1=enterfirstname");
}elseif($lastname == ""){
    header("Location:adminhome.php?err2=enterlasttname");
}elseif($email == ""){
    header("Location:adminhome.php?err3=enteremail");
}elseif($password == ""){
    header("Location:adminhome.php?err4=enterpassword");
}elseif($confirm_password == ""){
    header("Location:adminhome.php?err5=enterconfirmpassword");
    ...
    if($email==$row["email"]){
        header("Location:adminhome.php?err6=emailalreadyexist");   
        }else{
            //If all the above checking is surpassed then the users data is added to the database(i.e account created for new user)
        $insertquery="INSERT INTO `profile`( `first_name`,`last_name`,`email`,`password`,`confirm_password`,`profile_picture`,`status`,`score`,`user`) VALUES ('$firstname','$lastname','$email','$password','$confirm_password','$profile_picture','null',0,'staff')";
        mysqli_query($conn,$insertquery);
        //User redirected to login page with a successful message
        header("Location:adminhome.php?mssg2=SuccesfullyAdded");
        }
}else{
    //User redirected to login page with an error message
    header("Location:adminhome.php?err7=enteredpassworddoesnotmatch");
}
?>